 ...
 * @since 7.6.1
 */
public class KnownStore implements Determiner {
    ...
    public BigDecimal determine(Item item, Item having, PriceGuide soldGuide, PriceGuide offersGuide, PriceGuide offersDEGuide, StringBuilder remarks) {
        for (PriceDetail detail : offersGuide.getDetail()) {
            if ("TASTER BRICKS".equals(detail.getStore()) || "Bruno's Brick Depot".equals(detail.getStore())) {
                remarks.append(detail.getStore());
